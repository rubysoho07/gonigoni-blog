...
(2026-01-14 수정: 최근 권장되는 방식은 Flexible Orchestration 입니다. ([링크 참조](https://learn.microsoft.com/ko-kr/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-orchestration-modes)) 이를 바탕으로 Terraform 코드를 수정했습니다. 이전의 방식은 Unified Orchestration이고, Terraform에서는 `azurerm_linux_virtual_machine_scale_set`, `azurerm_windows_virtual_machine_scale_set` 리소스로 만듭니다.)
...
(2026-01-15 업데이트) Unified Orchestration 모드로 설정한 VMSS의 가상 머신들은, 위의 설명과 같이 모든 VM 목록에서 확인할 수 없고 생성한 VMSS에 들어가야 확인할 수 있습니다. 하지만 Flexible Orchestration 모드로 설정한 VMSS에 속한 가상 머신들은 모든 서버 목록에서 확인할 수 있습니다.
