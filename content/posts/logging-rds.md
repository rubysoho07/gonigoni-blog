...
{{< figure src="/img/rds-logging-1.png" caption="로그 찍기 설정(RDS 인스턴스 생성 시)" >}}

그리고 RDS 콘솔에 들어가면 로그 파일을 볼 수 있는데, 일반 로그나 느린 쿼리 로그를 찾을 수 없었다. 그래서 CloudWatch Logs를 찾아봤지만, 역시 로그가 없었다.

{{< figure src="/img/rds-logging-2.png" caption="로그 파일이 없다(-_-...)" >}}
...
{{< figure src="/img/rds-logging-3.png" caption="일반 로그(general log) 설정 변경" >}}
...
{{< figure src="/img/rds-logging-4.png" caption="일반 로그와 느린 쿼리 로그 파일이 생겼다!" >}}
